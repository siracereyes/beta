...
import { 
  ...
  Filter, Map, Building2, ClipboardList, Clock, Briefcase
} from 'lucide-react';
      ...
      <div className="px-10 py-8 border-b border-slate-100 bg-white sticky top-0 z-20">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8">
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-2 font-bold uppercase tracking-[0.15em] italic">Technical Assistance Oversight v4.9</p>
          </div>
                      ...
                      <div className="bg-white p-10">
                        ...
                        <div className="flex gap-4 mb-10 bg-slate-100 p-1.5 rounded-[1.8rem] w-fit border border-slate-200">
                          {[
                            ...
                            { id: 'signatories', label: 'Verification', icon: Users },
                        ...
                        <div className="animate-in fade-in slide-in-from-bottom-4 duration-400">
                          {activeTab === 'targets' && (
                            <div className="space-y-8">
                              <div className="bg-slate-900 rounded-[2rem] p-8 text-white mb-6">
                                ...
                                <p className="text-slate-400 text-xs mt-2 uppercase font-bold tracking-wider italic">Only active records with defined objectives are displayed below.</p>
                              </div>
                                      ...
                                      <div className="flex flex-col lg:flex-row">
                                        ...
                                        <div className="p-10 flex-grow">
                                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10">
                                            ...
                                            {/* 3. Due Date & 4. Status & 5. Help Needed */}
                                            ...
                                            {/* 8. TAP Due Date & 9. TAP Status */}
                                            <div className="space-y-6 bg-indigo-50/30 p-6 rounded-[2rem] border border-indigo-100/50">
                                              <div>
                                                <label className="text-[9px] font-black text-indigo-500 uppercase tracking-widest block mb-2">Finalization</label>
                                                <div className="space-y-4">
                                                  ...
                                                  <div>
                                                    <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-1">TAP Status Completion</label>
                                                    <div className={`text-[10px] font-black uppercase ${getStatusColor(target.tapStatus || '').replace('bg-', 'text-').replace('-100', '-600')}`}>
                                                      {target.tapStatus || '---'}
                                                    </div>
                                                  </div>
                                                </div>
                          ...
                          {activeTab === 'signatories' && (
                             <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                              <div className="space-y-4">
                                <h5 className="text-[11px] font-black uppercase tracking-widest text-indigo-600 px-4">Recipients</h5>
                                {record.receiverSignatories.map((sig, idx) => (
                                  <div key={idx} className="flex items-center gap-4 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                                    <div className="w-10 h-10 bg-indigo-50 text-indigo-600 flex items-center justify-center rounded-xl font-black text-xs">{idx+1}</div>
                                    <div>
                                      <p className="text-sm font-black text-slate-900">{sig.name}</p>
                                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">{sig.position}</p>
                                    </div>
                                  </div>
                                ))}
                              </div>
                              <div className="space-y-4">
                                <h5 className="text-[11px] font-black uppercase tracking-widest text-indigo-600 px-4">Providers</h5>
                                {record.providerSignatories.map((sig, idx) => (
                                  <div key={idx} className="flex items-center gap-4 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                                    <div className="w-10 h-10 bg-teal-50 text-teal-600 flex items-center justify-center rounded-xl font-black text-xs">{idx+1}</div>
                                    <div>
                                      <p className="text-sm font-black text-slate-900">{sig.name}</p>
                                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">{sig.position}</p>
                                    </div>
